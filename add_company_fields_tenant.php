 ...
 * Version pour système multi-tenant
 */
...
if(!isset($_SESSION['user_id'])) {
    die("❌ Erreur: Vous devez être connecté pour exécuter ce script.\n");
}

// Récupérer le nom de la base de données tenant
$tenant_database = $_SESSION['tenant_database'] ?? null;

if (!$tenant_database) {
    die("❌ Erreur: Aucune base de données tenant définie dans la session.\n");
}

echo "=== Ajout des nouveaux champs à la table companies ===\n";
echo "Base de données: $tenant_database\n\n";
...
if ($success_count > 0 || $skip_count > 0) {
    ...
    echo "\n🔄 Vous pouvez maintenant créer votre société.\n";
} else {
